...
public class VMon1 : MonoBehaviour
{
    Define.State nextBehavior = Define.State.ATTACKREADY;
    Define.PlayerMove nextDirection = Define.PlayerMove.Down;

    public int[] currentGridInd = null;  //현재 몬스터가 위치하고 있는 gridInd
    int[] moveGridInd = null;
    //private SpriteRenderer currentGridColor;   //현재 몬스터가 위치하고 있는 gridColor
    ...
    private void Start()
    {
        ...
        //List<Imfor> topSpawnArea = Managers.Field.GetMyField().getGridArray(Define.FieldArray.TOP);
        int rand = UnityEngine.Random.Range(1, Managers.Field.GetMyField().GetWidth() -1 );

        //Debug.Log(gameObject.name);
        //GameObject spawnGrid = topSpawnArea[rand].grid;
        Debug.Log("rand : " + rand);

        transform.position = Managers.Field.GetMyField().getAllGridArray()[rand][0].grid.transform.position;
        //transform.parent.transform.position = spawnGrid.transform.position;
        currentGridInd[0] = rand;
        currentGridInd[1] = 0;
    ...
    public void AutoWarningAttack(Define.PlayerMove nextDirection)
    {
        ...
        try
        {
            SpriteRenderer gridColor = gridArray[currentGridInd[0]][currentGridInd[1] + 1].grid.GetComponent<SpriteRenderer>();
